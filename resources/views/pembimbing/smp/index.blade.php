...
@endif
        ...
        <tbody>
            @foreach($users as $user)
                <tr>
                    ...
                    <td class="py-2 px-4 border-b">
                        <a href="{{ route('pembimbingsmp.edit', $user->id) }}" class="text-blue-600 hover:text-blue-900">Edit</a>
                        <form action="{{ route('pembimbingsmp.destroy', $user->id) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
    ...
    <div class="mt-4">
        <a href="{{ route('pembimbingsmp.create') }}" class="bg-blue-500 text-white px-4 py-2 rounded">Add User</a>
    </div>
</div>
@endsection
