...
<div class="bg-gray-100 p-0 sm:p-12">
    <div class="mx-auto max-w-2xl px-6 py-12 bg-white border-0 shadow-lg sm:rounded-3xl">
        <h1 class="text-2xl font-bold mb-8">Edit Ekskul</h1>
        ...
        <form action="{{ route('ekskul.update', $ekskul->id) }}" method="POST">
            ...
            <div class="relative z-0 w-full mb-5">
                <input
                    type="number"
                    name="kapasitas"
                    value="{{ old('kapasitas', $ekskul->kapasitas) }}"
                    placeholder=" "
                    required
                    class="pt-3 pb-2 block w-full px-0 mt-0 bg-transparent border-0 border-b-2 appearance-none focus:outline-none focus:ring-0 focus:border-black border-gray-200"
                />
            ...
            <div class="relative z-0 w-full mb-5">
                <input
                    type="number"
                    name="jumlah_peserta"
                    value="{{ old('jumlah_peserta', $ekskul->jumlah_peserta) }}"
                    placeholder=" "
                    class="pt-3 pb-2 block w-full px-0 mt-0 bg-transparent border-0 border-b-2 appearance-none focus:outline-none focus:ring-0 focus:border-black border-gray-200"
                />
                ...
                <div class="relative z-0 w-1/2 mb-5">
                    <select id="divisi_id" name="divisi_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                        ...
                        @foreach ($divisis as $divisi)
                            <option value="{{ $divisi->id }}" {{ $divisi->id == $ekskul->divisi_id ? 'selected' : '' }}>{{ $divisi->nama }}</option>
                        @endforeach
                    </select>
                </div>

                <div class="relative z-0 w-1/2 mb-5">
                    <select id="pembimbing_id" name="pembimbing_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                        <option value="">Pilih Pembimbing</option>
                        @foreach ($pembimbings as $pembimbing)
                            <option value="{{ $pembimbing->id }}" 
                                {{ $ekskul->pembimbing->contains('id', $pembimbing->id) ? 'selected' : '' }}>
                                {{ $pembimbing->name }}
                            </option>
                        @endforeach
                    </select>
